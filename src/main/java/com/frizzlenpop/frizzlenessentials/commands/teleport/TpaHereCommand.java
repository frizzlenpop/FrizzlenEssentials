...
public class TpaHereCommand extends BaseCommand {
    ...
    public TpaHereCommand(FrizzlenEssentials plugin) {
        super(plugin, "frizzlenessentials.tpahere", true);
    }
    ...
    protected boolean execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        // Send the teleport request (isHereRequest = true)
        boolean success = plugin.getTeleportManager().sendTeleportRequest(player, target, true);
        
        if (success) {
            MessageUtils.sendConfigMessage(player, "messages.teleport.request-here-sent", 
                    "Teleport here request sent to {player}.", "player", target.getName());
            
            MessageUtils.sendConfigMessage(target, "messages.teleport.request-here-received", 
                    "{player} has requested you to teleport to them. Type /tpaccept to accept or /tpdeny to deny.", 
